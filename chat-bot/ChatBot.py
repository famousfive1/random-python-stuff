...
for zzz in range(50):
    ...
    print(openers[index][0] + " " + openers[index][1], end=' ')

    pp = openers[index][1]
    np = graph[openers[index]]
    time = 0
    ...
    while time < 20:
        try:
            num = random.randint(0, len(np)-1)
            print(np[num], end=' ')
            # We get the next continuation set
            temp = graph[pp, np[num]]
